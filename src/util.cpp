...
const char * const BITCOIN_CONF_FILENAME = "the420coin.conf";
const char * const BITCOIN_PID_FILENAME = "the420coind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "the420coin" is a composite category enabling all The420Coin-related debug output
            if(ptrCategory->count(string("the420coin"))) {
...
#else
    const char* pszModule = "the420coin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\The420CoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\The420CoinCore
    // Mac: ~/Library/Application Support/The420CoinCore
    // Unix: ~/.the420coincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "The420CoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/The420CoinCore";
#else
    // Unix
    return pathRet / ".the420coincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty the420coin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override the420coin.conf
