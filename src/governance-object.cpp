...
bool CGovernanceObject::IsValidLocally(std::string& strError, bool& fMissingMasternode, bool& fMissingConfirmations, bool fCheckCollateral)
{
    ...
    // IF THE420COIN NO COUNT (NO-YES VALID VOTES) IS MORE THAN 10% OF THE NETWORK MASTERNODES, OBJ IS INVALID
        ...
        if((nObjectType == GOVERNANCE_OBJECT_TRIGGER) || (nObjectType == GOVERNANCE_OBJECT_WATCHDOG)) {
            ...
            if(!mnodeman.GetMasternodeInfo(vinMasternode.prevout, infoMn)) {
                ...
                if (err == CMasternode::COLLATERAL_OK) {
                    ...
                    strError = "Masternode UTXO should have 1000 FRTWY, missing masternode=" + strOutpoint + "\n";
                }
...
int CGovernanceObject::GetThe420CoinYesCount(vote_signal_enum_t eVoteSignalIn) const
{
    return GetYesCount(eVoteSignalIn) - GetNoCount(eVoteSignalIn);
}

int CGovernanceObject::GetThe420CoinNoCount(vote_signal_enum_t eVoteSignalIn) const
{
...
{
    return CountMatchingVotes(eVoteSignalIn, VOTE_OUTCOME_FRTWYTAIN);
}
...
void CGovernanceObject::UpdateSentinelVariables()
{
    ...
    if(GetThe420CoinYesCount(VOTE_SIGNAL_FUNDING) >= nAbsVoteReq) fCachedFunding = true;
    if((GetThe420CoinYesCount(VOTE_SIGNAL_DELETE) >= nAbsDeleteReq) && !fCachedDelete) {
        fCachedDelete = true;
        if(nDeletionTime == 0) {
            nDeletionTime = GetAdjustedTime();
        }
    }
    if(GetThe420CoinYesCount(VOTE_SIGNAL_ENDORSED) >= nAbsVoteReq) fCachedEndorsed = true;

    if(GetThe420CoinNoCount(VOTE_SIGNAL_VALID) >= nAbsVoteReq) fCachedValid = false;
}
