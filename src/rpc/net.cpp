...
UniValue getpeerinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"subver\": \"/The420Coin Core:x.x.x/\",  (string) The string version\n"
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8800\",  (string) The the420coin server IP and port we're connected to\n"
...
UniValue getnetworkinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"subversion\": \"/The420Coin Core:x.x.x/\",     (string) the server subversion string\n"
...
UniValue setban(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 ||
        ...
        throw runtime_error(
                            "setban \"ip(/netmask)\" \"add|remove\" (bantime) (the420coin)\n"
                            "\nAttempts add or remove a IP/Subnet from the banned list.\n"
                            "\nArguments:\n"
                            "1. \"ip(/netmask)\" (string, required) The IP/Subnet (see getpeerinfo for nodes ip) with a optional netmask (default is /32 = single ip)\n"
                            "2. \"command\"      (string, required) 'add' to add a IP/Subnet to the list, 'remove' to remove a IP/Subnet from the list\n"
                            "3. \"bantime\"      (numeric, optional) time in seconds how long (or until when if [the420coin] is set) the ip is banned (0 or empty means using the default time of 24h which can also be overwritten by the -bantime startup argument)\n"
                            "4. \"the420coin\"     (boolean, optional) If set, the bantime must be a the420coin timestamp in seconds since epoch (Jan 1 1970 GMT)\n"
                            "\nExamples:\n"
    ...
    if (strCommand == "add")
    {
        ...
        bool the420coin = false;
        if (params.size() == 4 && params[3].isTrue())
            the420coin = true;

        isSubnet ? g_connman->Ban(subNet, BanReasonManuallyAdded, banTime, the420coin) : g_connman->Ban(netAddr, BanReasonManuallyAdded, banTime, the420coin);
    }
    else if(strCommand == "remove")
    {
