...
*Setup instructions for a Gitian build of The420Coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the The420Coin
...
- [Building The420Coin Core](#building-the420coin-core)
...
For example, to connect as `root` from a Linux command prompt use
    ...
    Debian GNU/Linux comes with THE420COINLY NO WARRANTY, to the extent
...
Clone the git repositories for The420Coin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/the420coincrypto/the420coin
```
...
in the The420Coin Core repository under 'Fetch and build inputs' to install sources which require
...
To build The420Coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the The420Coin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/the420coin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/the420coincrypto/the420coin
...
./bin/gbuild --commit the420coin=${COMMIT} --url the420coin=${URL} ../the420coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit the420coin=${COMMIT} --url the420coin=${URL} ../the420coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit the420coin=${COMMIT} --url the420coin=${URL} ../the420coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the the420coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../the420coin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/the420coincrypto/the420coin-detached-sigs

BTCPATH=/some/root/path/the420coin.git
SIGPATH=/some/root/path/the420coin-detached-sigs.git

./bin/gbuild --url the420coin=${BTCPATH},signature=${SIGPATH} ../the420coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/the420coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/the420coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/the420coin-osx-build.assert
```
