...
Some notes on how to build The420Coin Core for Windows.
...
for Ubuntu Xenial does not produce working executables for some of the The420Coin Core applications.
...
Note that for WSL the The420Coin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/the420coincoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/the420coincrypto/the420coin.git
    sudo chmod -R a+rw the420coin
    cd the420coincoin
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/the420coincrypto/the420coin.git
    sudo chmod -R a+rw the420coincoin
...
way. This will install to `c:\workspace\racone`, for example:
    ...
    make install DESTDIR=/mnt/c/workspace/the420coincoin
...
causes two of the the420coincoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the the420coincoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the the420coincoin source code).