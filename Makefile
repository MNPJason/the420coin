...
pkgdatadir = $(datadir)/the420coincore
pkgincludedir = $(includedir)/the420coincore
pkglibdir = $(libdir)/the420coincore
pkglibexecdir = $(libexecdir)/the420coincore
...
CONFIG_HEADER = $(top_builddir)/src/config/the420coin-config.h
CONFIG_CLEAN_FILES = libthe420coinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libthe420coinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/the420coin-config.h.in \
...
ACLOCAL = ${SHELL} /media/sf_jerry/coindev/the420coin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /media/sf_jerry/coindev/the420coin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /media/sf_jerry/coindev/the420coin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /media/sf_jerry/coindev/the420coin/build-aux/missing automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /media/sf_jerry/coindev/the420coin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = the420coincore
PACKAGE_BUGREPORT = https://github.com/the420coincrypto/the420coincoin/issues
PACKAGE_NAME = The420Coin Core
PACKAGE_STRING = The420Coin Core 0.12.2
PACKAGE_TARNAME = the420coincore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /media/sf_jerry/coindev/the420coin
abs_srcdir = /media/sf_jerry/coindev/the420coin
abs_top_builddir = /media/sf_jerry/coindev/the420coin
abs_top_srcdir = /media/sf_jerry/coindev/the420coin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /media/sf_jerry/coindev/the420coin/build-aux/install-sh
...
pkgconfig_DATA = libthe420coinconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/the420coind$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/the420coin-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/the420coin-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = The420Coin-Qt.app
OSX_DMG = The420Coin-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_the420coin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_the420coin_coverage.info test_the420coin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/The420Coin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/the420coin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/the420coin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/the420coin-config.h
$(top_srcdir)/src/config/the420coin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/the420coin-config.h src/config/stamp-h1
libthe420coinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libthe420coinconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/The420Coin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/The420Coin-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "The420Coin-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/The420Coin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_the420coin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_the420coin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_the420coin_filtered.info: test_the420coin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_the420coin_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_the420coin_filtered.info
...
#test_the420coin_coverage.info: baseline_filtered_combined.info test_the420coin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_the420coin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_the420coin_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_the420coin_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_the420coin.coverage/.dirstamp:  test_the420coin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_the420coin.coverage/.dirstamp total.coverage/.dirstamp

# The420Coin: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_the420coin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
